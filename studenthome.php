...
<html>
	...
	<body>
		...
		<div id="chgpwd">
			...
			<form name="chgpwd" action="studenthome.php" method="post" onsubmit="return chgpwdValid();">
				<table cellpadding="10" class="reqtable">
		...
		<?php
			if(isset($_POST['oldpwd'])) {
				$oldpwd = $_POST['oldpwd'];
				$query = "select *from studentlogin where password='$oldpwd'";
				$result = $con -> query($query);
				$count = $result -> num_rows;
				if($count == 0) { ?>
					<script type="text/javascript">
				...
				<?php
					...
					$newpwd = $_POST['newpwd'];
					$query = "update studentlogin set password='$newpwd' where password='$oldpwd'";
					$result = $con -> query($query);
					if($result) { ?>
						<script type="text/javascript">
